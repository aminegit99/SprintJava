 ...
 * @author THEOLDISBACK
 */
public class modifierbacController implements Initializable {
        ...
        if (ab == 0) {
            ...
            if (checkString(ref.getText())) {
                if (adresse.getText().equals("")) {
                    ...
                    s.modifierbac(new Bac(thisbac.getId(), ref.getText(), adresse.getText(), cod, cap, etat1));
                    Alert a = new Alert(Alert.AlertType.INFORMATION, "votre bac est modifier ");
                    a.show();
    ...
    public void setData(Bac newbac) {
        this.thisbac = newbac;
